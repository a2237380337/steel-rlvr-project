"""Train baseline or tail-aware Dr. GRPO from the same SFT checkpoint."""
...
def _prepare_dataset(config: dict[str, Any]) -> Any:
    ...
    if missing:
        raise ValueError(f"GRPO dataset is missing columns: {missing}")
    dataset = dataset.select_columns(REWARD_COLUMNS)
    invalid_frequencies = [
        int(value) for value in dataset["grade_frequency"] if int(value) <= 0
    ]
    if invalid_frequencies:
        raise ValueError("GRPO training rows must have positive training-only grade frequencies")
    return dataset


def _reward_functions(config: dict[str, Any]) -> list[Any]:
    if bool(config["tail_aware"]):
        primary = partial(
            tail_aware_value_reward,
        ...
        primary = value_reward
    return [primary, strict_json_reward, physical_range_reward]
...
def main() -> None:
    ...
    if not torch.cuda.is_available() or torch.version.hip is None:
        raise SystemExit("Dr. GRPO requires ROCm/HIP PyTorch")
    output_dir, run_manifest, metrics_callback = prepare_training_run(
        run_name=f"grpo-{args.config.stem}",
